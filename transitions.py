...
class TransitionParser:
  ...
  def _count_transitions(self):
    ...
    for observation in self.observations:
      for state_transition in observation['state_transitions']:
       ...
       # print("state" + str(state) + "action" + str(action) + "state_" + str(state_))
       # print ("TRANSITION COUNT: " + str(transition_count [state][action][state_]))
  ...
  def _parse_probabilities(self, transition_count):
    ...
    return P
    #P(s,a,s')=(nb of times we too action a and ended up in s')/nb of times we took action s)